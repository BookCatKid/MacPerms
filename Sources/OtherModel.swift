...
enum OtherPane: String, CaseIterable, Identifiable, Hashable {
    ...
    var helpText: String {
        ...
        case .loginItems:
            return "The 'Open at Login' list — System Settings → General → Login Items & Extensions → Open at Login.\n\nTwo kinds of entries appear here: SMAppService login items (launchd services — Enable/Disable maps to `launchctl enable|disable gui/<uid>/<label>`) and app records whose BTM disposition is enabled (the classic open-at-login entries). Remove deletes the BTM record outright — enabled app entries are also removed via System Events. Live apps may re-register on next launch."
        case .backgroundItems:
            return "Launch agents/daemons and app background activity — managed by backgroundtaskmanagementd via binary .btm files. This is System Settings → General → Login Items & Extensions → Allow in the Background.\n\nsfltool exposes no per-item toggle, but every launchd-backed item's enabled state lives in `launchctl print-disabled` — Enable/Disable writes that registry directly (state shown per row; launchd value wins when present). Remove deletes the record surgically from the .btm archive (unsupported but verified) — a live app may re-register on next launch.\n\n'Reset ALL' runs sfltool resetbtm — the nuclear option: every item re-registers on next launch."
        case .appExtensions:
