...
enum OtherPane: String, CaseIterable, Identifiable, Hashable {
    ...
    var helpText: String {
        ...
        case .loginItems:
            return "The 'Open at Login' list — System Settings → General → Login Items & Extensions → Open at Login.\n\nEnable/Disable writes the same disposition bits the Settings toggle writes (ON sets enabled+allowed, OFF clears allowed); Enable additionally clears any stale `launchctl disable` override so the service can actually run. Remove deletes the BTM record outright — enabled app entries are also removed via System Events. Live apps may re-register on next launch."
        case .backgroundItems:
            return "Launch agents/daemons and app background activity — managed by backgroundtaskmanagementd via binary .btm files. This is System Settings → General → Login Items & Extensions → Allow in the Background.\n\nEnable/Disable writes the same disposition bits the Settings toggle writes — ON sets enabled+allowed, OFF clears only allowed — via .btm archive surgery, cascading to child records (agents, tasks, plugins under the app). Enable also clears any stale launchd override on launchd-backed items so the service can actually run. Remove deletes the record outright. All are unsupported-but-verified edits requiring SIP disabled — a live app may re-register on next launch."
        case .appExtensions:
